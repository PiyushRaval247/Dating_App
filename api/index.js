...
// List activity dates for a user's streak (YYYY-MM-DD)
app.get('/activity/dates', authenticateToken, async (req, res) => {
  try {
    ...
    const dates = items
      .map(it => it?.activityDate?.S)
      .filter(Boolean)
      .sort();
    return res.status(200).json({ dates });
  } catch (error) {
    console.log('Error fetching activity dates', error);
    return res.status(500).json({ message: 'Internal server error' });
  }
});
